...
{
    public class OptMarkingAssistanceSimulator
    { public class ReportArgument : EventArgs
        ...
        public OptMarkingAssistanceSimulator(Deployment deployment, NetworkTopology topology, SQLiteUtility sql, string version)
        {
        ...
        public void Run(int attackPacketPerSec, int normalPacketPerSec, int totalPacket, int percentageOfAttackPacket, double probibilityOfPacketTunneling, double probibilityOfPacketMarking, double startFiltering, int initialTimeOfAttackPacket, bool dynamicProbability, bool considerDistance)
        {
            ...
            foreach (NetworkTopology.Node node in topology.Nodes)
            {
                if (node.Tracer == NetworkTopology.TracerType.Tunneling)
                {
                    ...
                    node.IsTunnelingActive = false;
                }
            }

            Console.WriteLine("Active node:");
            foreach (NetworkTopology.Node node in topology.Nodes.Where(n => n.IsTunnelingActive == true && n.Tracer == NetworkTopology.TracerType.Tunneling))
            ...
            for (int i = 0; i < totalPacket; i++)
            {
                ...
                foreach (int nodeId in deployment.TunnelingTracerID.Where(nodeId => path.Contains(nodeId)))
                {
                    topology.Nodes.First(n => n.ID == nodeId && n.Tracer == NetworkTopology.TracerType.Tunneling).IsTunnelingActive = true;
                }
                ...
                for (int j = 0; j < path.Count; j++)
                {
                    ...
                    switch (topology.Nodes[topology.NodeID2Index(path[j])].Tracer)
                    {
                        case NetworkTopology.TracerType.Tunneling:
                            if (!isTunneling && !isMarking && packetEvent.Type == NetworkTopology.NodeType.Attacker /*rd.NextDouble() <= topology.Nodes.Find(node => node.ID == path[j]).ProbabilityOfTunneling*/ && topology.Nodes.Find(node => node.ID == path[j]).IsTunnelingActive)
                            {
